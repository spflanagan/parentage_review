...
slmnp<-lapply(gty.files,function(file){
  dir<-paste("SOLOMON/",gsub("_genotypes.txt","",file),sep="")
  outname<-paste(dir,gsub("genotypes.txt","",file),sep="/")
  gty<-read.delim(file)
...
```{r}
snppit<-lapply(gty.files,function(file){
  outname<-paste("SNPPIT/",gsub("genotypes","snppit",file),sep="")
  gty<-read.delim(file)
  f<-cervus2snppit(gty=gty,filename = outname)
})
snppit.mst<-lapply(mst.files,function(file){
  outname<-paste("SNPPIT/",gsub("microsatellites","ms_snppit",file),sep="")
  gty<-read.delim(file)
  f<-cervus2snppit(gty=gty,filename = outname)
})
```
...
```{r}
peds<-lapply(gty.files,cervus2plink)
```
...
```
```{r}
mb<-lapply(gty.files,function(gfile){
  ...
  res1<-expression(varPed(x="Mom",gender="Female",relational="OFFSPRING",restrict="=="))#offspring match to males
