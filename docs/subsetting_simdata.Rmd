...
Gs<-lapply(gty.files,function(file){
  outname<-paste("MasterBayes/",gsub("genotypes.txt","",file),sep="")
  gty<-read.delim(file)
  G<-cervus2colonyG(gty=gty,filename = outname)
})
Ps<-lapply(Gs,cervus2colonyP,sexes=c(Males="MAL",Females="FEM",Offspring="OFF"))
```
