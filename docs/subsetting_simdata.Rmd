...
SNPPIT only works on SNPs, so we won't convert the microsatellites.
...
cervus_correct<-function(gty.files){
  prop<-do.call(rbind,lapply(gty.files,function(filename){
    # get the cervus-assigned dads
    csvname<-gsub("genotypes.txt","paternity.csv",filename)
...
csvs<-list.files(path = "cervus",pattern=".csv",full.names = TRUE)
```
