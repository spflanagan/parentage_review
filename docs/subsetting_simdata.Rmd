...
lapply(gty.files,function(gfile){
...
mb<-lapply(gty.files[4:length(gty.files)],function(gfile){
  ...
  model<-MCMCped(PdP=PdP, GdP=GdP, verbose=FALSE,burnin=50000,nitt=55000)
...
```{r}
colony.summ<-function(gfile,replace_string="_genotypes.txt"){
  dir<-paste("colony",gsub(replace_string,"",gfile),sep="/")
  assigns<-read.csv(paste(dir,gsub(replace_string,".ParentPair",gfile),sep="/"))
