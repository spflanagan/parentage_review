...
### COLONY and MasterBayes

```{r}
Gs<-lapply(gty.files,function(file){
  outname<-paste("MasterBayes/",file,sep="")
  G<-cervus2colonyG(file)
  write.table(G,outname,row.names = FALSE,col.names = TRUE,sep='\t',quote=FALSE)
  P<-cervus2colonyP(G,sexes=c(Males="MAL",Females="FEM",Offspring="OFF"))
  p.outname<-paste("MasterBayes/",gsub("genotypes","phenotypes",file),sep="")
...
```
```{r}
Gs<-lapply(gty.files,cervus2colonyG)
Ps<-lapply(Gs,function(G){
  cervus2colonyP(G$id,sexes=c(Males="MAL",Females="FEM",Offspring="OFF"))
})

mb<-mapply(function(G,P){
  GdP<-GdataPed(G=G, categories=NULL)
  sP<-startPed(estG=FALSE, A=extractA(G), E1=0.005, E2=0.005,estUSsire=FALSE, USsire=10) 
...
PdP<-PdataPed(formula=list(res1,res2),data=PipefishP,USdam=TRUE)
  model<-MCMCped(PdP=P, GdP=GdP, sP=sP, verbose=FALSE)
  return(model)
},Gs,Ps)
